...
class Login:
    ...
    def crear_interfaz_login(self):
        ...
        tk.Button(self.ventana_login, text="Ingresar", command=self.validar_login).pack(pady=5)
        tk.Button(self.ventana_login, text="Registrar nuevo usuario", command=self.registrar_usuario).pack(pady=5)

    def validar_login(self):
        usuario = self.entry_usuario.get()
        contrasena = self.entry_contrasena.get()
        usuario_id = self.usuarios_db.validar(usuario, contrasena)
        if usuario_id:
            messagebox.showinfo("Éxito", "Inicio de sesión correcto")
            self.ventana_login.destroy()
            ventana_principal = VentanaPrincipal(usuario_id)
    ...
    def registrar_usuario(self):
        ...
        if usuario == "" or contrasena == "":
            ...
            return
        usuario_id = self.usuarios_db.agregar_usuario(usuario, contrasena)
        if usuario_id:
